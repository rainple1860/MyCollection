...
public class MyHasMap<K,V> {
    ...
    public boolean putAll(MyHasMap<K,V> map){
    ...
    private void resize(int newThreshold) {
        ...
        for (int i = 0; i < table.length; i++) {
            ...
            if (node != null) {
                table[i] = null;
                newTable[node.hash & (newThreshold - 1)] = node;
            }
        }
        table = newTable;
    }
    ...
    private int hash(Object key){
        ...
        h = hasCode ^ (hasCode >>> 20) ^ (hasCode >>> 12);
        return key == null ? 0 : h ^ (h >>> 7) ^ (h >>> 4);*/
       int h;
    ...
    public Node<K,V>[] entrySet(){
        ...
        for (Node<K, V> node : table) {
            if (node != null){
                if (node instanceof Tree){
                    MyArrayList<Node> all = ((Tree) node).getTreeNodes();
